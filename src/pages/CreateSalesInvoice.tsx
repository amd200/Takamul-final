                            ...
                            <div key={item.id}>
                              <div className={cn("grid grid-cols-1 gap-3 p-4 md:p-2 bg-muted/40 md:bg-transparent rounded-xl md:rounded-none border md:border-none border-border items-center group", headerCols)}>
                                ...
                                <Controller
                                  ...
                                  render={({ field, fieldState }) => (
                                    <Field>
                                      <Input type="number" readOnly={!allowPriceChangeOnSale} value={field.value == 0 ? "" : field.value} onChange={(e) => field.onChange(Number(e.target.value))} className={`text-center ${!allowPriceChangeOnSale ? "cursor-not-allowed opacity-70" : ""}`} />
                                      {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
                                    </Field>
                                  )}
                                />
