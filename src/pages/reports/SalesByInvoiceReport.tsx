...
import { useGetSalesInvoicesReport } from "@/features/reports/hooks/Usegetsalesinvoicesreport";
...
export default function SalesByInvoiceReport() {
  ...
  // Data Fetching
  const { data: rawSalesData, isLoading: salesLoading, isFetching: salesFetching } = useGetSalesInvoicesReport({
  ...
  const salesData = isSearched ? (rawSalesData ?? []) : [];
  ...
  const totalSales = useMemo(() => salesData.reduce((s, r) => s + (r.subTotal || 0), 0), [salesData]);
  const totalTax = useMemo(() => salesData.reduce((s, r) => s + ((r.grandTotal || 0) - (r.subTotal || 0)), 0), [salesData]);
  const totalFinal = useMemo(() => salesData.reduce((s, r) => s + (r.grandTotal || 0), 0), [salesData]);

  const title = t("sales_by_invoice", "تقرير إجمالي المبيعات على مستوى أرقام الفواتير");
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {!isExempt && (
              <FinancialStatCard
                title={t("total_sales_without_tax", "إجمالي المبيعات بدون ضريبة")}
