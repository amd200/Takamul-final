      ...
      {hasAnyPermission([Permissions?.salesOrders?.view, Permissions?.expenses?.view, Permissions?.purchaseOrders?.view]) && (
        <div className="space-y-4 relative dashboard-page" dir={direction}>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {hasPermission(Permissions?.statistics?.sales) && <StatCard title={t("total_sales")} value={`${formatCurrency(totalSalesValue, { ...systemSettings, money: { ...systemSettings.money, showCurrencySymbol: false } })}`} icon={SaudiRiyal} bgClass="bg-[var(--dashboard-sales)]" textColor="text-[var(--dashboard-sales-text)]" delay={0.1} isLoading={salesLoading} />}
            {hasPermission(Permissions?.statistics?.expenses) && <StatCard title={t("total_expenses")} value={formatCurrency(totalExpensesValue, { ...systemSettings, money: { ...systemSettings.money, showCurrencySymbol: false } })} icon={Banknote} bgClass="bg-[var(--dashboard-expenses)]" textColor="text-[var(--dashboard-expenses-text)]" delay={0.2} isLoading={expensesLoading} />}
            {hasPermission(Permissions?.statistics?.profit) && <StatCard title={t("net_profit")} value={formatCurrency(netProfitValue, { ...systemSettings, money: { ...systemSettings.money, showCurrencySymbol: false } })} icon={TrendingUp} bgClass="bg-[var(--dashboard-profit)]" textColor="text-[var(--dashboard-profit-text)]" delay={0.3} onClick={() => navigate("/reports/profit")} isLoading={profitLoading} />}
